...
class LlamaOnnxDmlInterface(BaseLLMInterface):
    ...
    def initialize(self):
        ...
        self.llm_provider = self.llm_session.get_providers()[0]
        if self.llm_provider == "DmlExecutionProvider":
            self.binding_device = "dml"
        if self.llm_provider == "CUDAExecutionProvider":
            self.binding_device = "cuda"
        if self.llm_provider == "CPUExecutionProvider":
...
short answers are usually best"
        ...
        for x in history[::-1]:
            ...
            if len(self.tokenizer.encode(prompt + history_text + x, bos=True, eos=False)) <= max_length:
                ...
                break
        if flag:
            return prompt + history_text, np.array(
