...
def run_ONNX_LlamaV2(
    ...
    tokenizer = Tokenizer(model_path=TokenizerPath)
    tokens = tokenizer.encode(prompt, bos=True, eos=False)

    # create the embedding layer.
    embeddingLayer = torch.nn.Embedding(tokenizer.n_words, hidden_size)
    embeddingLayer.load_state_dict(torch.load(embedding_file))
    embeddingLayer.eval()

    # Create the embeddings of the initial prompt.
    x = embeddingLayer(torch.tensor(tokens)).detach().cpu().numpy()
    ...
    for idx in range(max_gen_len):
        ...
        x = embeddingLayer(torch.tensor(next_token)).unsqueeze(0)
